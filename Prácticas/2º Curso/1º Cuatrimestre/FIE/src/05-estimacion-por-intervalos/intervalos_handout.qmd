...
```{r}
# Completar aquí
# Calcular la media y el margen de error al 95% para cada variable
resumen_ic <- ess_data_ES_long |> 
  group_by(variable) |>
  summarise(
    media = mean(valor, na.rm = TRUE),
    margen_error_95 = calcular_error_ic(valor, 0.05)
  ) |>
  pivot_wider(
    names_from = variable,
    values_from = c(media, margen_error),
    names_glue = "{variable}_{.value}"
  ) |>
  rename(
    ppltrst_media = ppltrst_media,
    ppltrst_error = ppltrst_margen_error,
    pplfair_media = pplfair_media,
    pplfair_error = pplfair_margen_error,
    pplhlp_media = pplhlp_media,
    pplhlp_error = pplhlp_margen_error,
  )
