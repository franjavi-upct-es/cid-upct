...
def criterio(nivel, s, M, A, capacidades, mecanicos_usados, mejor_reparadas, soluciones):
    ...
    # Poda temprana: Si no quedan suficientes niveles para alcanzar una mejor solución
    reparadas_actuales = sum(1 for x in s if x > 0)
    if reparadas_actuales + (A - nivel) <= mejor_reparadas[0]:
        return

    # Heurística: Ordenar mecánicos en función de capacidades para mejorar eficiencia
    # Seleccionamos los mecánicos que tienen capacidad para la avería actual y no han sido usados
    candidatos = [m for m in range(M) if capacidades[m][nivel] == 1 and not mecanicos_usados[m]]
    candidatos.sort(key=lambda x: sum(capacidades[x]), reverse=True)  # Orden descendente de capacidades

    # Intentar asignar mecánicos a la avería actual
    for mecanico in candidatos:
        s[nivel] = mecanico + 1
