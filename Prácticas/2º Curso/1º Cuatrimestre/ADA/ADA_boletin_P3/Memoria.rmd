...
num_pruebas = 30  # Número de pruebas a realizar
...
El algoritmo tiene una complejidad temporal lineal respecto al tamaño de la cadena **`A`** y la longitud de las subcadenas **`m`**, ya que recorren **`n - m + 1`** subcadenas y, para cada una, se realiza un recorrido de longitud **`m`**. Por lo tanto tiene una complejidad temporal de $O(n\cdot m)$.
...
1.  **Casos base:**
    ...
    -   **Cuando el segmento es menor que `m`:**
        -   Si \textbf{\texttt{r - l + 1 < m}}, significa que el segmento no es lo suficientemente grande para contener una subcadena de longitud **`m`**.
        -   En este caso, el algoritmo devuelve **`(-1, 0)`**, indicando que no hay una subcadena válida en este segmento.
    -   **Cuando el segmento es exactamente de longitud `m`:**
        -   Si \textbf{\texttt{r - l + 1 == m}}, podemos procesar este segmento directamente dentro de la función.
        -   Realizamos el conteo de apariciones consecutivas de **`C`** en la subcadena y devolvemos el resultado.
...
def analyse_complexity_divide_y_venceras(test_cases):
    ...
    if overall_time:
        plt.figure()
        plt.plot(range(1, iteration), overall_time, marker='o', color='red')
        plt.xlabel("Iteraciones (Casos)")
        plt.ylabel("Tiempo Acumulado (s)")
        plt.title("Tiempo Acumulado de divide_y_venceras por Iteración")
        plt.show()
    else:
        print("No se procesaron casos.")
```
...
El algoritmo divide la cadena en mitades logarítmicamente y en cada nivel realiza un trabajo lineal, lo que resultado en una complejidad de $O(n\log n)$.
...
test_cases = generar_casos_de_prueba(30, 100, 'c', alfabeto)
...
def comparar_algoritmos_tiempo_acumulado(test_cases):
    ...
    plt.plot(iteraciones, tiempos_directo_acumulado, marker='o', color='blue', label='resolver_directo')
    plt.plot(iteraciones, tiempos_divide_acumulado, marker='s', color='green', label='divide_y_venceras')
    plt.xlabel("Iteraciones (Casos)")
    plt.ylabel("Tiempo Acumulado (s)")
