...
frecuencias_ejemplo1 <- tibble(
  observadas = c(34, 10, 20), # frecuencias observadas
  teoricas = c(9 / 16, 3 / 16, 4 / 16),
  esperadas = teoricas * sum(observadas) # probabilidades bajo la hipotesis nula
)
```
...
# Completar aquí
chi <- sum(
  (frecuencias_ejemplo1$observadas - frecuencias_ejemplo1$esperadas)^2 /
    frecuencias_ejemplo1$esperadas
)
...
p_valor <- pchisq(
  q = chi,
  df = nrow(frecuencias_ejemplo1) - 1,
  lower.tail = FALSE
)

str_glue("El p-valor asociado al contraste es {round(p_valor, 4)}")
# Fin Completar aquí
```

-   que decisión tomaríamos?

En realidad, `R` tiene implementada la función `chisq.test` que admite como input un vector con los valores de $f_i$ y otro vector con los valores de $p_i$:
...
chi2sq_valores <- map_dbl(
  lista_simulaciones,
  \(x) chisq.test(x, p = c(9 / 16, 3 / 16, 4 / 16))$statistic
)
...
```{r}
DATA_DIRECTORY <- "../data/"
iberdrola <- read_csv(paste0(DATA_DIRECTORY, "Iberdrola.txt"), col_names = "precio")
...
```{r}
# Completar aquí
gof_std <- gofstat(ajuste)

tibble(
  chi2 = gof_std$chisq,
  p_valor = gof_std$chisqpvalue
)
# fin de Completar aquí
```
...
Un servidor Web corporativo recibe tareas (peticiones para documentos HTML). Cada página es una petición interna de la corporación intranet (`1`) o una externa de internet (`2`). Se ha observado el tiempo transcurrido hasta la llegada de la décima petición en periodos de tiempo independientes. El fichero `peticiones_tareas.csv` contiene los datos del tiempo transcurrido hasta la llegada de la décima petición de cada tarea.

El conjunto de datos incluye las siguientes columnas:

`ID`: Identificador único de cada experimento.

`t_llegada`: Tiempo, en segundos, transcurrido hasta la décima petición.

`tipo_proceso`: Tipo de proceso, `1` (Intranet), `2` (Internet).
:::

Importar los datos del fichero `peticiones_tareas.csv` en un dataframe llamado `datos_tareas`.

```{r}
#| warning: false
DATA_DIRECTORY <- "../data/"
datos_tareas <- read_csv(file = paste0(DATA_DIRECTORY, "peticiones_tareas.csv"))
```

Considerando que los datos de tiempo transcurrido siguen una distribución gamma, obtener una estimación de máxima verosimilitud de los parámetros de esta distribución.

-   Usar para ello la función `fitdist` (usar `help(fitdis)` para obtener ayuda).

-   Introducir como valores iniciales (opción `start`) los estimadores por el método de los momentos que llamaréis `alphahat0` y `betahat0`:

```{r}
#| warning: false
x <- datos_tareas$t_llegada
xbar <- mean(x)
s2 <- mean(x^2) - xbar^2
alphahat0 <- xbar^2 / s2
betahat0 <- s2 / xbar

valores_iniciales <- list(shape = alphahat0, scale = betahat0)
```
...
ajuste_gamma <- fitdist(
  data = na.omit(x),
  distr = "gamma",
  start = valores_iniciales
)

summary(ajuste_gamma)
```

### Contrastar la hipótesis de que los datos siguen una distribución gamma.

```{r}
# Completar aquí
gof_gamma <- gofstat(ajuste_gamma)

tibble(
  chi2 = gof_gamma$chisq,
  gl = gof_gamma$chisqdf,
  p_valor = gof_gamma$chisqpvalue
)
# Fin Completar aquí
```

### completa el estudio con una análisis gráfico
