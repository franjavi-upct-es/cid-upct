...
```{r}
# nada que completar
t.test(P, y = NULL, alternative = "less", mu = 2)
```
...
```{r}
# completar aquí
ic_P_99 <- t.test(P, y = NULL, conf.level = 0.99)
ic_P_99
...
```{r}
tccb <- c(
  0.22, 0.33, 0.42, 0.51, 0.60, 0.76, 1.13, 0.23, 0.34, 0.43, 0.52, 0.62,
...
```{r}
# completar aquí
shapiro_tccb <- shapiro.test(tccb)
shapiro_tccb
# Fin de completar aquí
```
...
shapiro_log_tccb <- shapiro.test(log.tccb)
shapiro_log_tccb
# Fin de completar aquí
```
...
binom_less <- binom.test(223, 228, p = 0.95, alternative = "less")
binom_less
# Fin de completar aquí
```
...
binom_ic95 <- binom.test(223, 228, conf.level = 0.95)
binom_ic95
# Fin de completar aquí
```
...
prop_less <- prop.test(223, 228, p = 0.95, alternative = "less")
prop_less
# Fin de completar aquí
```
...
```{r}
# completar aquí
prop_ic95 <- prop.test(223, 228, conf.level = 0.95)
prop_ic95
# Fin de completar aquí
```
...
```{r}
# completar aquí
boxplot(rend.varA, xlim = c(0, 3), at = 1, ylim = c(6, 14))
boxplot(rend.varB, add = TRUE, at = 2)
axis(1, at = c(1, 2), labels = c("Variedad A", "Variedad B"))
...
shapiro_A <- shapiro.test(rend.varA)
shapiro_A
# Fin de completar aquí
```
...
shapiro_B <- shapiro.test(rend.varB)
shapiro_B
# Fin de completar aquí
```
...
```{r}
# completar aquí
constraste_var <- var.test(rend.varA, rend.varB)
constraste_var
...
tt_medias <- t.test(rend.varA, rend.varB, alternative = "two.sided", var.equal = TRUE)
tt_medias
...
mw <- wilcox.test(rend.varA, rend.varB, alternative = "two.sided")
mw
# Fin de completar aquí
```
...
tt_pareado <- t.test(sleep.D, sleep.L, alternative = "two.sided", paired = TRUE)
tt_pareado
# Fin de completar aquí
```
...
ic_pareado_99 <- t.test(sleep.D, sleep.L, conf.level = 0.99, paired = TRUE)
ic_pareado_99
# Fin de completar aquí
```
...
```{r}
# completar aquí
wilcox_pareado <- wilcox.test(sleep.D, sleep.L, alternative = "two.sided", paired = TRUE)
wilcox_pareado
# Fin de completar aquí
```
...
fisher_ic <- fisher.test(ejemplo6, conf.level = 0.95)
fisher_ic
...
fisher_contraste <- fisher.test(ejemplo6, alternative = "two.sided")
fisher_contraste
# Fin de completar aquí
```
...
prop_2m <- prop.test(c(72, 64), c(80, 80), alternative = "two.sided")
prop_2m
# Fin de completar aquí
```
...
```{r}
# Completar aquí
prop_2m_ic <- prop.test(c(72, 64), c(80, 80), conf.level = 0.95)
prop_2m_ic
# fin de completar aquí
```
...
Un servidor Web corporativo recibe tareas (peticiones para documentos HTML). Cada página es una petición interna ("1") de la corporación intranet o una externa ("2") de internet. Se ha observado el tiempo transcurrido hasta la llegada de la octava petición en periodos de tiempo independientes. El conjunto de datos que consideramos `peticiones.csv` contiene datos de tiempo transcurrido hasta la llegada de octava petición de una tarea.

Contiene las siguientes columnas:

`task_ID`: Identificador único de cada experimento.

`t_llegada`: tiempo en segindos transcurrido hasta la décima petición.

`tipo_proceso`: Tipo de proceso (`1` (Intranet), `2` (Internet).
:::
...
library(tidyverse)
DATA_DIRECTORY <- "../data/"
datos_tareas <- read_csv(file = paste0(DATA_DIRECTORY, "peticiones.csv"))
```


Consideramos que los datos de la columna `tipo_proceso` , `tipo_proceso == "1"` y , `tipo_proceso == "2"`, corresponden a m.a.s de dos poblaciones normales independientes $\mathcal{N}(\mu_X, \sigma^2_X)$ y $\mathcal{N}(\mu_Y, \sigma^2_Y)$. Contrastar si la media tipo proceso intranet es igual a la media del tipo de proceso internet.


```{r}
x <- datos_tareas |>
  filter(as.character(tipo_proceso) == "1") |>
  pull(t_llegada) |>
  na.omit()

y <- datos_tareas |>
  filter(as.character(tipo_proceso) == "2") |>
  pull(t_llegada) |>
  na.omit()

# H0: mu_X = mu_Y, H1: mu_X != mu_Y
contraste_var <- var.test(x, y)
contraste_var

constraste_medias <- t.test(
  x, y,
  alternative = "two.sided",
  var.equal = (contraste_var$p.value > 0.05)
)
constraste_medias
```
