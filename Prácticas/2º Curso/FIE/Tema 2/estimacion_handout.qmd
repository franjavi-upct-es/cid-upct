...
simular_muestras <- function(n_muestras, n, alpha, beta) {
  # Completar aquí
  map(seq_len(n_muestras), \(i) rgamma(n, shape = alpha, scale = beta))
  # Fin Completar aquí
}
set.seed(314159L)
simular_muestras(n_muestras = 5L, n = 4L, alpha = 3L, beta = 2L)
```
...
calcular_estimadores_momentos <- function(x) {
  ...
  alphahat <- xbar^2L / (x2bar - xbar^2L)
  betahat <- (x2bar - xbar^2L) / xbar
...
set.seed(314159L)
# Completar aquí
lista_muestras <- simular_muestras(
  n_muestras = 5L,
  n = 100L,
  alpha = 3L,
  beta = 2L
...
calcular_sesgo_mse <- function(valores_estimadores, alpha_real, beta_real) {
  valores_estimadores |>
    summarise(
      ...
      mse_alpha = mean((alphahat - alpha_real)^2L),
      sesgo_beta = mean(betahat - beta_real),
      mse_beta = mean((betahat - beta_real)^2L)
    )
}
# Fin Completar aquí
valores_estimadores |>
  calcular_sesgo_mse(alpha_real = 3L, beta_real = 2L)
```
...
calcular_flujo_completo <- function(
  n_muestras, n, alpha_real, beta_real, calcular_estimadores
) {
  simular_muestras(
    ...
    map_dfr(calcular_estimadores) |>
    calcular_sesgo_mse(alpha_real = alpha_real, beta_real = beta_real) |>
    mutate(n = n)
}
...
resultados_momentos |>
  select(n, sesgo_alpha, sesgo_beta) |>
  pivot_longer(
    cols = c(sesgo_alpha, sesgo_beta),
    names_to = "estimador",
    values_to = "sesgo"
  ) |>
  mutate(
    estimador = recode(
      estimador,
      sesgo_alpha = "Sesgo de alphahat",
      sesgo_beta = "Sesgo de betahat"
    )
  ) |>
  ggplot(aes(x = n, y = sesgo, color = estimador)) +
  geom_line(linewidth = 1L) +
  geom_point(size = 2L) +
  scale_x_log10() +
  labs(
    x = "Tamaño muestral n (escala log10)",
    y = "Sesgo aproximado",
    color = "Estimador"
  ) +
  theme_minimal()
...
```{r}
alpha0 <- (mean(x))^2L / (mean(x^2L) - (mean(x))^2L)
optimize(
  \(alpha) calcular_logverosimilitud_alpha(alpha = alpha, x),
  interval = c(0L, 2L * alpha0),
  maximum = TRUE
)
```
...
calcular_estimadores_mv <- function(x) {
  xbar <- mean(x)
  x2bar <- mean(x^2L)
  alpha0 <- xbar^2L / (x2bar - xbar^2L)

  opt <- optimize(
    \(alpha) calcular_logverosimilitud_alpha(alpha = alpha, x = x),
    interval = c(1e-8, max(2L * alpha0, 1e-3)),
    maximum = TRUE
  )

  alphahat <- opt$maximum
  betahat <- xbar / alphahat
...
resultados_momentos <- map_dfr(
  ...
  \(n) calcular_flujo_completo(
    ...
    mutate(metodo = "Metodo de los momentos")
)

resultados_mv <- map_dfr(
  c(10, 100, 1000, 10000),
  \(n) calcular_flujo_completo(
    n_muestras = 10000,
    n = n,
    alpha_real = 3,
    beta_real = 2,
    calcular_estimadores_mv
  ) |>
    mutate(metodo = "Maxima verosimilitud")
)

bind_rows(resultados_momentos, resultados_mv) |>
  arrange(n, metodo)
# Fin Completar aquí
```
