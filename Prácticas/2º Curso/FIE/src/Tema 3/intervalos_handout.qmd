...
Cada dos años, se realiza desde 2002 una gran encuesta llamada [European Social Survey] cuyo objetivo es medir las actitudes, creencias y patrones de comportamiento de las poblaciones de más de 30 naciones.
...
Para restringir las columnas en el mismo momento de la carga del fichero, se usa el parámetro `col_select` de `read_csv`, ver la [documentación]

  [documentación]: https://readr.tidyverse.org/reference/read_delim.html
...
`across` (ver [documentación][1]) permite aplicar una o varias funciones a columnas de un dataframe:

  [1]: https://dplyr.tidyverse.org/reference/across.html

-   Su primer argumento especifica qué columnas consideramos, para ello, usamos la sintaxis de `tidy-select`, ver la [documentación][2]\
...
-   el argumento `.names` indica cómo se construyen los nombres de las nuevas columnas con una sintaxis inspirada en `glue`, ver [documentación][3].

  [2]: https://dplyr.tidyverse.org/reference/dplyr_tidy_select.html
  [3]: https://glue.tidyverse.org/
...
Si queremos aplicar la misma función lógica sobre varias columnas de un dataframe, podemos usar `if_all` o `if_any` según si queremos comprobar una intersección o una unión de las condiciones, ver [documentación][documentación1]. Son de la misma familia que `across`. Podéis probar a usarlo en el caso anterior.
:::

  [documentación1]: https://dplyr.tidyverse.org/reference/across.html
...
Para ello, se usa `pivot_longer` de la librería "tidyr", ver [documentación][4]

  [4]: https://tidyr.tidyverse.org/reference/pivot_longer.html
...
# 1. Comprobar independencia mediante visualización
ess_data_ES |>
  ggplot(aes(x = pplfair, y = pplhlp)) +
  geom_jitter(shape = 21, color = "red", fill = "red", alpha = 0.5, size = 2) +
  xlim(0, 10) +
  ylim(0, 10) +
  labs(
    title = "Relación entre pplfair y pplhlp",
    x = "pplfair (honradez)",
    y = "pplhlp (ayuda a los demás)"
  )

# 2. Calcular correlación para evaluar dependencia
correlacion <- cor(ess_data_ES$pplfair, ess_data_ES$pplhlp, use = "complete.obs")
str_glue("La correlación entre pplfair y pplhlp es: {round(correlacion, 4)}")

# 3. Crear variable diferencia
ess_data_ES <- ess_data_ES |>
  mutate(diferencia_fair_hlp = pplfair - pplhlp)

# 4. Calcular IC al 98% para la diferencia de medias
alpha <- 0.02  # Para confianza del 98%

ic_diferencia_fair_hlp <- ess_data_ES |>
  summarise(
    media_diferencia = mean(diferencia_fair_hlp, na.rm = TRUE),
    margen_error = calcular_error_ic(diferencia_fair_hlp, alpha)
  ) |>
  mutate(
    limite_inferior = media_diferencia - margen_error,
    limite_superior = media_diferencia + margen_error
...
ic_diferencia_fair_hlp

str_glue(
  "IC al 98% para (pplfair - pplhlp): [{round(ic_diferencia_fair_hlp$limite_inferior, 4)}, {round(ic_diferencia_fair_hlp$limite_superior, 4)}]"
)
...
# 1. Filtrar los datos para obtener las respuestas de hombres y mujeres a la pregunta 'pplhlp'
pplhlp_hombres <- ess_data_ES |> filter(gndr == 1) |> pull(pplhlp)
pplhlp_mujeres <- ess_data_ES |> filter(gndr == 2) |> pull(pplhlp)

# 2. Calcular IC al 98% para la diferencia de medias (sin suponer varianzas iguales)
alpha <- 0.02  # Para confianza del 98%

media_dif_pplhlp <- mean(pplhlp_hombres, na.rm = TRUE) - mean(pplhlp_mujeres, na.rm = TRUE)
margen_error_pplhlp <- calcular_error_diferencia(
  pplhlp_hombres, 
  pplhlp_mujeres, 
  alpha, 
...
str_glue(
  "IC al 98% para (mu_hombres - mu_mujeres) en pplhlp: {round(media_dif_pplhlp, 4)} ± {round(margen_error_pplhlp, 4)}"
)

str_glue(
  "Intervalo: [{round(media_dif_pplhlp - margen_error_pplhlp, 4)}, {round(media_dif_pplhlp + margen_error_pplhlp, 4)}]"
)

# 3. Calcular IC al 98% para el cociente de varianzas
ic_cociente_var_pplhlp <- calcular_ic_cociente_varianzas(pplhlp_hombres, pplhlp_mujeres, alpha)

ic_cociente_var_pplhlp

str_glue(
  "IC al 98% para el cociente de varianzas (sigma²_hombres / sigma²_mujeres): [{round(ic_cociente_var_pplhlp$izq, 4)}, {round(ic_cociente_var_pplhlp$der, 4)}]"
)

# 4. Análisis complementario: verificar si el 1 está en el IC del cociente de varianzas
contiene_uno <- ic_cociente_var_pplhlp$izq <= 1 & ic_cociente_var_pplhlp$der >= 1

str_glue(
  "¿El IC del cociente de varianzas contiene el valor 1? {ifelse(contiene_uno, 'SÍ', 'NO')}"
)
