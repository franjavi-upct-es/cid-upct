...
summary(motor_forward)
summary(motor_backward)

# Comparar los R-cuadrados ajustados
cat("R-cuadrado ajustado del modelo forward: ", summary(motor_forward)$adj.r.squared, "\n")
cat("R-cuadrado ajustado del modelo backward: ", summary(motor_backward)$adj.r.squared, "\n")

# Comparar los AIC
cat("AIC del modelo forward: ", AIC(motor_forward), "\n")
cat("AIC del modelo backward: ", AIC(motor_backward), "\n")

# Comparar los BIC
cat("BIC del modelo forward: ", BIC(motor_forward), "\n")
cat("BIC del modelo backward: ", BIC(motor_backward), "\n")

```

El `motor_backward` parece ser el mejor modelo, ya que tiene el R-cuadrado ajustado más alto y el AIC y BIC más bajos.
...
```{r}
predict(motor_backward, newdata = data.frame(VRP = 2000, VRS = 19000, LN_RFC = 10.3089, PRESION = 180, TEMP_ESC = 1700, TEMP_AMB = 95), interval = "confidence", level = 0.95)
```
