...
def main():
    ...
    dfConHora = (
        dfMarcas
        .withColumn("event_ts", to_timestamp(col("event_time"), "yyyy-MM-dd HH:mm:ss 'UTC'"))
    ...
    dfAgrupado = (
        dfConHora
        .groupBy("brand", "Hora")
        .agg(count("*").alias("NInteracciones"))
    )
    ...
    dfResultado = (
        dfAgrupado
        .select(col("brand").alias("Marca"), col("Hora"), col("NInteracciones"))
