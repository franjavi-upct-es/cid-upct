...
\item \lb{Resolver el siguiente problema con programación dinámica. Tenemos un conjunto de \textbf{\texttt{n}} objetos, cada uno con un peso $\mathbf{\mathtt{p=(p_1,p_2,...,p_n)}}$. El objetivo es repartir los objetos entre dos montones diferentes, de manera que queden lo más equilibrados posible en peso. Esto es, se debe minimizar la diferencia entre los pesos totales de ambos montones. Aplciar sobre el ejemplo con \textbf{\texttt{n=4}} y \textbf{\texttt{p=(2,1,3,4)}}.} 
  ...
  Sea \textbf{\texttt{dp[i][w]}} un valor booleano que indica si es posible obtener una suma \textbf{\texttt{w}} usando los primeros \textbf{\texttt{i}} objetos.

  \textbf{Ecuación de recurrencia} 

  \begin{enumerate}[label=\arabic*)]
      \item Si no incluimos el objeto \textbf{\texttt{i}}: \[
              \mathbf{\mathtt{dp[i][w]=dp[i-1][w]}}
      \] 
  \item Si incluimos el objeto \textbf{\texttt{i}} (siempre que $\mathbf{\mathtt{w\ge p[i])}}$: \[
          \mathbf{\mathtt{dp[i][w]=dp[i-1][w]}}\text{ o }\mathbf{\mathtt{dp[i-1][w-p[i]}}
  \] 
  \textbf{Condiciones iniciales}
  \begin{enumerate}[label=\arabic*)]
  \item \textbf{\texttt{dp[0][0]=True}} (es posible tener suma 0 usando 0 objetos).
  \item \textbf{\texttt{dp[0][w]=False}} para $\mathbf{\mathtt{w>0}}$ (no es posible tener una suma positiva sin objetos).
  \end{enumerate}

  \textbf{Reconstrucción de la solución}

  Al terminar la tabla \textbf{\texttt{dp}}, buscamos el mayor $\mathbf{\mathtt{w\le \left\lfloor \dfrac{P}{2} \right\rfloor}}$ tal que \textbf{\texttt{dp[n][w]=True}}. Este valor corresponde a $\mathbf{\mathtt{\mathrm{sum}(S_1)}}$, y el otro subconjunto tendrá suma $\mathbf{\mathtt{P-\mathrm{sum}(S_1)}}$.

  \textbf{Implementación en Python}

  \lstinputlisting{"Ejercicio 8.py"}

  \begin{verbatim}
## Diferencia mínima: 0
## Montón 1: 5, Montón 2: 5
  \end{verbatim}

  \textbf{Complejidad temporal:} $O\left( n\cdot l\left\lfloor \dfrac{P}{2} \right\rfloor \right)  $, donde $n$ es el número de objetos y $P$ es la suma total de los pesos.
\end{enumerate}
\item \lb{Nos vamos de compras al mercado. Tenemos \textbf{\texttt{K}} euros en el bolsillo y una lista de \textbf{\texttt{m}} productos que podemos comprar. Cada producto tiene un precio $\mathbf{\mathtt{p_i}}$ (que será simpre un número entero), y una utilidad, $\mathbf{\mathtt{u_i}}$. De cada producto podemos comprar como máximo 3 unidades. Además, tenemos una oferta según según la cual la segunda unidad nos cuesta 1 euro menos, y la tercera 2 euros menos. Queremos elegir los productos a comprar, maximizando la utilidad de los productos comprados. Resolver el problema por programación dinámica, indicando la ecuación recurrente, con sus casos base, las tablas, el algoritmo para rellenarlas y la forma de componer la solución a partir de las tablas.}
    ...
    \textbf{Algoritmo para llenar la tabla}
    \begin{enumerate}[label=\arabic*)]
    \item Inicializa la tabla \textbf{\texttt{dp}} con ceros.
    \item Itera por cada producto \textbf{\texttt{i}}.
    \item Para cada producto, itera desde el presupuesto máximo \textbf{\texttt{K}} hacia abajo para evitar reutilización indebida de los valores.
    \item Para cada \textbf{\texttt{j}} en el presupuesto, calcular \textbf{\texttt{dp[j]}} usando la ecuación de recurrencia.
    \end{enumerate}
    \textbf{Reconstrucción de la solución}
    \begin{enumerate}[label=\arabic*)]
    \item A partir de \textbf{\texttt{dp[K]}}, identifica qué productos y cuántas unidades de cada contribuyen a la utilidad máxima.
    \item Usa una tabla auxiliar para rastrear decisiones, o simplemente simula hacia atrás verificando qué opciones generaron la utilidad.
    \end{enumerate}
    \textbf{Implementación en Python} 
    \lstinputlisting{"Ejercicio 9.py"}
    \begin{verbatim}
## Máxima utilidad: 15
## Productos comprados (unidades por producto): [2, 1]
    \end{verbatim}
    \textbf{Complejidad temporal:} $O(m\cdot K)$, donde $m$ es el número de productos y $K$ es el presupuesto.
\item \lb{Resolver el siguiente problema usando programación dinámica. Dada una secuencia de enteros positivos $\mathbf{\mathtt{(a_1,a_2,a_3,\dots,a_n)}}$, encontrar la subsecuencia creciente más larga de elementos no necesariamente consecutivos. Es decir, encontrar una subseuencia $\mathbf{\mathtt{(a_{i1}, a_{i2}, \dots,a_{ik})}}$, con $\mathbf{\mathtt{(a_{i1}< a_{i2}< \cdots< a_{ik})}}$ y $\mathbf{\mathtt{(1\le i1<i2<\cdots<ik\le n)}}$. Por ejemplo, para la siguiente secuencia la solución sería longitud 6 (formada por los números señalados en negrita): 3, \textbf{1}, 3, \textbf{2}, \textbf{3}, 8, \textbf{4}, 7, \textbf{5}, 4, \textbf{6}.}
    ...
    Dada una secuencia de enteros $\mathbf{\mathtt{a=[a_1,a_2,\dots,a_n]}}$, queremos encontrar la longitud de la \textbf{subsecuencia creciente más larga} y, opcionalmente, la subsecuencia en sí. 
    ...
    \textbf{Implementación en Python} 
    \lstinputlisting{"Ejercicio 10.py"}
    \begin{verbatim}
