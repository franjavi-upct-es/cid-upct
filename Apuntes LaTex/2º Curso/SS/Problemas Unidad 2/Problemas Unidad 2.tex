...
\begin{enumerate}[label=\color{red}\textbf{\alph*)}]
    \item \db{Indique razonadamente si el anterior sistema cumple las propiedades de memoria, causalidad, estabilidad, invertibilidad, linealidad y/o invarianza temporal.}
      ...
      \begin{itemize}[label=\textbullet]
        \item \textbf{Memoria:}
            ...
            El sistema tiene memoria, ya que la salida $y[n]$ depende de una suma de muestras de la entrada  $x[\cdot ]$ evaluadas en instantes anteriores (en particular, todos los $x[m]$ con  $m\le n-2$ después del cambio de variable que se verá más abajo). No se basa únicamente en el valor de $x[n]$ en el instante actual.
        \item \textbf{Causalidad:}
    ...
    \item \db{Utiliza la definición de convolución, obtenga la salida del sistema del enunciado, $y[n]$, cuando a su entrada se tiene la señal  $x[n]=(n+1)u[n]$.}
        ...
        La suma  \[
        \sum_{m=0}^{n-2} (m+1)
        \] es la suma de los primeros $n-1$ números naturales (empezando en 1), y se conoce que  \[
        1+2+\cdots+(n-1)=\dfrac{n(n-1)}{2}.
        \] 
        Por lo tanto, la respuesta en el tiempo es \[
            y[n]=\dfrac{n(n-1)}{2}u[n-2]
        \] 
        \begin{center}
            \begin{tikzpicture}
                \begin{axis}[
                    ...
                    axis lines = middle,
                    xlabel style={at={(axis cs:10,0)}, anchor=west},
                    ylabel style={at={(axis cs:0,11)}, anchor=south},
                ]
                    \pgfmathsetmacro{\mysamples}{10-(-10)+1}
                    \addplot+[ycomb, mark=*, lightblue, line width=1.5, domain=-10:10, samples=\mysamples] {(x+1)*u(x)};
                \end{axis}
            \end{tikzpicture}\qquad
            \begin{tikzpicture}
                \begin{axis}[
                    ...
                    axis lines = middle,
                    xlabel style={at={(axis cs:10,0)}, anchor=west},
                    ylabel style={at={(axis cs:0,45)}, anchor=south},
                ]
                    \pgfmathsetmacro{\mysamples}{10-(-10)+1}
                    \addplot+[ycomb, mark=*, lightblue, line width=1.5, domain=-10:10, samples=\mysamples] {((x*(x-1))/2)*u(x-2)};
                \end{axis}
            \end{tikzpicture}
        \end{center}
...
\begin{enumerate}[label=\textbf{Paso \arabic*:}]
    \item \textbf{Respuesta al impulso equivalente del sistema}
        ...
        Dado que $h_1[n]$ es no nula solo en $n=1$, la convolución es simplificada a:  \[
            h[n]=\sum_{k=-\infty}^{\infty} h_1[k]h_2[n-k]=h_1[1]h_2[n-1].
        \] 
        Como $h_1[1]=1$, se tiene: \[
            h[n]=h_2[n-1].
        \] 
        Sustituyendo $h_2[n-1]$: \[
            h[n]=\left( \dfrac{1}{3} \right) ^{(n-1)-1}u[(n-1)+1]=\left(\dfrac{1}{3}\right)^{n-2}u[n].
        \] 
        \begin{center}
        	\begin{tikzpicture}[scale=0.7]
        		\begin{axis}[
        			...
        			\pgfmathsetmacro{\mysamples}{5-(-5)+1}
        			\addplot+[ycomb, mark=*, lightblue, line width=1.5, domain=-5:5, samples=\mysamples] {x!=0 ? 0 : 1};
        		\end{axis}
        	\end{tikzpicture}
           \qquad
            \begin{tikzpicture}[scale=0.7]
                \begin{axis}[
                    ...
                    axis lines = middle,
                    xlabel style={at={(axis cs:5,0)}, anchor=west},
                    ylabel style={at={(axis cs:0,9)}, anchor=south},
                ]
                    \pgfmathsetmacro{\mysamples}{5-(-5)+1}
                    \addplot+[ycomb, mark=*, lightblue, line width=1.5, domain=-5:5, samples=\mysamples] {((1/3)^(x-1))*u(x+1)};
                \end{axis}
            \end{tikzpicture}\qquad
            \begin{tikzpicture}[scale=0.7]
                \begin{axis}[
                    ...
                    \pgfmathsetmacro{\mysamples}{5-(-5)+1}
                    \addplot+[ycomb, mark=*, lightblue, line width=1.5, domain=-5:5, samples=\mysamples] {((1/3)^(x-2))*u(x)};
                \end{axis}
            \end{tikzpicture}
        \end{center}
            ...
            \begin{center}
                \begin{tikzpicture}
                    \begin{axis}[
                        ...
                        \pgfmathsetmacro{\mysamples}{5-(-5)+1}
                        \addplot+[ycomb, mark=*, lightblue, line width=1.5, domain=-5:5, samples=\mysamples] {(-1)^x*(u(2-x)-u(-x-1))};
                    \end{axis}
                \end{tikzpicture}
            \end{center}
            \textbf{Convolución $y[n]$:}

            Sustituyendo $h[n]=\left( \dfrac{1}{3} \right) ^{n-2}u[n]$ y $x[n]$ en la convolución:  \[
                y[n]=\sum_{k=-\infty}^{\infty} x[k]h[n-k].
            \] 
            Dado que $x[k]$ es no nula solo para  $-1<k\le 2$, la suma se reduce a: \[
                y[n]=\sum_{k=0}^{2} x[k]h[n-k].
            \] 
            Sustituendo $h[n-k]=\left( \dfrac{1}{3} \right) ^{(n-k)-2}u[n-k]$, la convolución se convierte en: \[
                y[n]=\sum_{k=0}^{2} x[k]\left( \dfrac{1}{3} \right) ^{n-k-2}u[n-k].
            \] 
            El término $u[n-k]$ asegura que solo se suman términos para los cuales  $n\ge k$. Por lo tanto, la suma final es: \[
                y[n]=\sum_{k=0}^{\min(2,n)}x[k]\left( \dfrac{1}{3} \right) ^{n-k-2}. 
            \] 
            Sustituyendo $x[k]=(-1)^k$ para  $k=0,1,2$, se obtiene:  \[
                y[n]=\sum_{k=0}^{\min(2,n)}(-1)^k\left( \dfrac{1}{3} \right) ^{n-k-2} 
            \] 
            \begin{center}
            	\begin{tikzpicture}
                \begin{axis}[
                    ...
                    \pgfmathsetmacro{\mysamples}{10-(-10)+1}
                    \addplot+[ycomb, mark=*, lightblue, line width=1.5, domain=-10:10, samples=\mysamples] coordinates {
                    	(-10,0)
                    	(-9,0)
                    	(-8,0)
                    	(-7,0)
        ...
        (3,-3+1/3-1/9) 
        (4,-3+1/3-1/9+1/27) 
        (5,-3+1/3-1/9+1/27-1/81)
        (6,-3+1/3-1/9+1/27-1/81+1/243)
        (7,0)
        (8,0)
        (9,0)
        (10,0)
...
\item \lb{Dibuje el diagrama de bloques de los siguientes sistemas LTI causales descritos mediante sus ecuaciones en diferencias:}
    \begin{enumerate}[label=\color{red}\textbf{\alph*)}]
        \item \db{$y[n]=\dfrac{1}{3}y[n-1]+\dfrac{1}{2}x[n]$} 
            \begin{center}
                \begin{tikzpicture}[ >=latex,
                   ...
                   \node (x) at (0,0) {$x[n]$};
                   \node [sum, right=of x] (sum) {\small $\sum$};
                   \node [block, right=of sum] (gain1) {\small $\tfrac{1}{2}$};
                   \node [block, below=of gain1] (gain2) {\small $\tfrac{1}{3}$};
                   \node [block, right=of gain2] (delay) {\small $z^{-1}$};
                   \node[right= 4cm of gain1] (y) {$y[n]$};
                   
                   \draw [->] (x) -- (sum) node[midway, above] {};
                   \draw [->] (sum) -- (gain1);
                   \draw [->] (gain1) -- (y);
                   \draw [->] (y) |- (delay);
                   \draw [->] (delay) -- (gain2);
        ...
        \begin{tikzpicture}[node distance=1.5cm, >=latex,
        	...
        	node distance=1.5cm and 1.5cm]
        	\node (x) at (0,0) {$x[n]$};
        	\node [block] (delay1) at (2,0) {\small $z^{-1}$};
        	\node [sum] (sum) at (4,0) {\small $\sum$};
        	\node [block] (gain1) at (6,0) {\small $1$};
        	\node [block] (gain2) at (6,-2) {\small $\frac{1}{3}$};
