...
tibble(p = p_muestras) |>
  ...
  labs(title = expression(paste("Distribución de ", hat(p),
                                ", 10000 muestras"))) +
  theme_bw()
```
...
x <- seq(qnorm(0.01, muphat, sigmaphat),
         qnorm(0.99, muphat, sigmaphat), length = 200)
...
hist(xsim, xlim = c(-3, 3),
     breaks = "Sturges", main = "",
     xlab = "Valores muestrales",
     ylab = "", col = "lightgreen")
```
...
curve(dgamma(x, 2, 3), from = 0, to = 3,
      xlab = "Distribución gamma", ylab = " ", ylim = c(-0.1, 1.5))
abline(0, 0)
xsim <- rgamma(100, 2, 3)
y <- 0
for (i in 1:100){
...
hist(xsim, xlim = c(0, 3), breaks = "Sturges", main = "",
     xlab = "Valores muestrales", ylab = "", col = "lightblue")
```
...
crear_diana <- function(puntos, titulo) {
  ...
  ggplot() +
    geom_circle(data = circulos, aes(x0 = x0, y0 = y0, r = r), color = "black", linewidth = 0.4) +
...
#### ¿Cuándo observamos $S(t)\ge 100$€? #### {.unnumbered .unlisted}
```{r}
...
#### Generación de valores de una distribución uniforme #### {.unnumbered .unlisted}
```{r}
...
#### Generación de valores de una distribución gamma #### {.unnumbered .unlisted}
```{r}
...
#### Transformación de una variable aleatoria #### {.unnumbered .unlisted}
:::{.callout-note}
...
#### Función características #### {.unnumbered .unlisted}
...
#### Desigualdades #### {.unnumbered .unlisted}
:::{.callout-note}
