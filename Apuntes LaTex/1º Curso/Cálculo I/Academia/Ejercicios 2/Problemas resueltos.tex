...
\[ \lim_{x\to1}f(x)=\begin{cases}
	...
	\lim_{x\to1^+}\dfrac{\sin\left(\frac{1}{x-1}\right)}{e^{\frac{1}{x-1}}+1}=\dfrac{\sin\left(\frac{1}{0^+}\right)}{e^{\frac{1}{0^+}}+1}=\dfrac{\sin(+\infty)}{e^{+\infty}+1}=\dfrac{\sin(+\infty)}{+\infty}=0\text{ (Teorema del Sándwich)}
\end{cases} \]
...
\end{minipage}\qquad\begin{minipage}{0.45\textwidth}
\begin{tikzpicture}
	\node[red, draw=red, fill=red!10, line width=1.5, text width=\linewidth] {\underline{Nota:} Para representar gráficamente una parábola $y=ax^2+bx+c$, debemos siempre tener en cuenta lo siguiente:
...
\begin{itemize}
	\item Como el coeficiente líder es negativo (-1) entonces la parábola es $\bigcap$.
...
\item \lb{Analiza la continuidad de las siguientes funciones \[ q(x)=\begin{cases}
		-2x+1 & x\le0\\
		2x+3 & 0<x\le2\\
		\dfrac{1}{x-2} & x\ge2
	\end{cases} \]}
\begin{itemize}[leftmargin=*]
	...
	\begin{center}
		...
		\lim_{ x \to 0^- }(-2x+1)=1\\
		\lim_{x\to0^+}(2x+3)
	\end{rcases}~~$\begin{minipage}{0.6\textwidth}
	No es continua en $x=0$, en concreto presenta una discontinuidad inevitable de salto finito o de 1ª Especie.
	\end{minipage}
	\end{center}
	\item[] \underline{Veamos en $x=2$:}
	\begin{center}
		$\begin{rcases}
			\lim_{x\to2^-}(2x+3)=7\\
			\lim_{x\to2^+}\dfrac{1}{x-2}=\dfrac{1}{0^+}=+\infty
		\end{rcases}~~$\begin{minipage}{0.6\textwidth}
		No es continua en $x=2$, en concreto presenta una discontinuidad inevitable de salto infinito o de 2ª especie. Habría una asíntota vertical en $x=2$.
		\end{minipage}
	\end{center}
\end{itemize}
\item \lb{Calcular la derivada de la siguiente función: \[ f(x)=x^{\sin x} \]}

\begin{minipage}[b]{0.45\textwidth}
	$\log f(x)=\log(x^{\sin x})=\sin x\cdot\log x$\\
	Derivo:\\
	\[ \begin{array}{l}
		\dfrac{1}{f(x)}\cdot f'(x)=\cos x\cdot\log x+\sin x\cdot\dfrac{1}{x}\\
		f'(x)=f(x)\left[\cos x\log x+\dfrac{\sin x}{x}\right]\\
		\bboxed{f'(x)=x^{\sin x}\left[\cos x\log x+\dfrac{\sin x}{x}\right]}
	\end{array} \]
\end{minipage}\qquad\begin{minipage}[t]{0.45\textwidth}
\begin{tikzpicture}
	\node[red, draw=red, fill=red!10, line width=1.5, text width=\linewidth] {\underline{Nota:} Si tenemos que derivar una función en forma exponencial, donde, tanto base como exponente dependen de $x,\:\left(f(x)\right)^{g(x)}$. Lo que haremos será meter logaritmo y bajar el exponente. Derivaremos y despejaremos la derivada. Este tipo de  derivación se llama \textbf{Derivación logarítmica}.};
\end{tikzpicture}
\end{minipage}
\item \lb{Calcular la derivada de la siguiente función: \[ f(x)=(\sin x) \]}
\begin{minipage}{0.45\textwidth}
	Realizaremos la derivada de esta función mediante derivación logarítmica: \[ \log f(x)=\log\left(\sin x\right)^{\log x}=\log x\cdot\log(\sin x) \]Por lo tanto, nos queda: \[ \log(f(x))=\lbb{\log x}{}\cdot\lbb{\log(\sin x)}{} \]Derivamos en ambos lados de la ecuación: \[ \begin{array}{l}
		\dfrac{1}{f(x)}\cdot f'(x)=\dfrac{1}{x}\cdot\log(\sin x)+\log x\cdot\dfrac{1}{\sin x}\cdot\cos x=\dfrac{\log(\sin x)}{x}+\log x\cdot\dfrac{\cos x}{\sin x}\\
		f'(x)=f(x)\left[\dfrac{\log(\sin x)}{x}+\log x\cdot\dfrac{\cos x}{\sin x}\right]\\
		\bboxed{f'(x)=\left(\sin x\right)^{\log x}\cdot\left[\dfrac{\log(\sin x)}{x}+\log x\cdot\dfrac{\cos x}{\sin x}\right]}
	\end{array} \]
\end{minipage}\qquad\begin{minipage}[t]{0.45\textwidth}
\begin{tikzpicture}
	\node[red, draw=red, fill=red!10, line width=1.5, text width=\linewidth] {\underline{Nota:} Si tenemos que derivar una función en forma exponencial, donde, tanto base como exponente dependen de $x,\:\left(f(x)\right)^{g(x)}$. Lo que haremos será meter logaritmo y bajar el exponente. Derivaremos y despejaremos la derivada. Este tipo de  derivación se llama \textbf{Derivación logarítmica}.};
\end{tikzpicture}
\end{minipage}
\item \lb{Calcular la recta tangente a la curva dada por $f(x)=x^x$ en $x=1$.}

\begin{minipage}{0.45\textwidth}
	Como queremos hallar la recta tangente, entonces:
	\[ \begin{array}{l}
		y-b=m(x-a)\begin{cases}
			b=f(a)\\
			m=f'(a)
		\end{cases}\\
		\bboxed{a=1}\\
		b=f(1)=1^1=1\longrightarrow \bboxed{b=1}
	\end{array} \]
	Para hallar la derivada, debemos aplicar derivación logarítmica: \[ \log f(x)=\log(x^x)=x\log x \]
	Derivo: 
	\[ \begin{array}{l}
		\dfrac{1}{f(x)}\cdot f'(x)=\log x+x\cdot\dfrac{1}{x}=1+\log x\longrightarrow f'(x)=f(x)(1+\log x)\\
		\bboxed{f'(x)=x^x\left[1+\log x\right]}\longrightarrow f'(1)=1^1\left[1+\tozero{\log 1}~~~\right]=1\longrightarrow\bboxed{m=f'(1)=1}
	\end{array} \]
	Por lo tanto, la recta tangente viene dada por: \[ y-1=1\cdot(x-1)\longrightarrow \bboxed{y=x} \]
\end{minipage}\qquad\begin{minipage}[t]{0.45\textwidth}
\begin{tikzpicture}
	\node[red, draw=red, fill=red!10, line width=1.5, text width=\linewidth] {\underline{Nota:} Dada una curva $y=f(x)$, para hallar la recta tangente a dicha curva en el punto $x=a$, debemos obtener la expresión: \[ \begin{array}{cc}
			\text{Recta tangente} & \text{Recta normal}\\
			\boxed{\begin{array}{c}
					y-b=m(x-a)\\
					b=f(a)\\
					m=f'(a)
...
\end{tikzpicture}
\end{minipage}
\item \lb{Determinar la derivada de $h(x)=x^{x^2+1}$.}

Como la expresión de $h(x)$ es una exponencial donde tanto la base como el exponente dependen de $x$, entonces debemos derivar esta expresión mediante derivación implícita: \[ \log(h(x))=\log\left(x^{x^2+1}\right)=(x^2+1)\cdot\log x \]Derivando: \[ \begin{array}{l}
	\dfrac{1}{h(x)}\cdot h'(x)=2x\log x+(x^2+1)\cdot\dfrac{1}{x}=2x\log x+\left(x+\dfrac{1}{x}\right)\\
	h'(x)=h(x)\left[2x\log x+\left(x+\dfrac{1}{x}\right)\right]=x^{x^2+1}\left[2x\log x+x+\dfrac{1}{x}\right]\\
	\bboxed{h'(x)=x^{x^2+1}\left[2x\log x+x+\dfrac{1}{x}\right]}
\end{array} \]
\item \lb{Utilizando las reglas de L'Hôpital vamos a calcular los siguientes límites \[ \lim_{x\to 0}\dfrac{\sin(3x)}{e^{x}-1} \]}
\begin{minipage}[t]{0.5\textwidth}
	$\lim_{x\to0}\dfrac{\sin(3x)}{e^x-1}=\left(\dfrac{0}{0}\right)=\left\{\text{L'Hôpital}\right\}=\lim_{x\to0}\dfrac{3\cos(3x)}{e^x}=\bboxed{3}$
\end{minipage}\qquad\begin{minipage}{0.4\textwidth}
\begin{tikzpicture}
	\node[red, draw=red, fill=red!10, line width=1.5, text width=\linewidth] {\underline{Nota:} Siempre que tengamos un límite que sea el cociente de dos funciones y nos de una indeterminación de la forma $\left(\dfrac{\infty}{\infty}\right)$ o $\left(\dfrac{0}{0}\right)$ podemos plantearnos resolverlo mediante la regla de L'Hôpital: \[ \lim_{x\to a}\dfrac{f(x)}{g(x)}=\left(\dfrac{0}{0}\text{ ó }\dfrac{\infty}{\infty}\right)=\lim_{x\to a}\dfrac{f'(x)}{g'(x)} \]};
\end{tikzpicture}
\end{minipage}
\item \lb{Utilizando las reglas de L'Hôpital vamos a calcular los siguientes límites: \[ \lim_{x\to0}\dfrac{x-\sin (x)}{3x^3+2x^4} \]}

$\lim_{ x \to 0 }\dfrac{x-\sin(x)}{3x^{3}+2x^{4}}=\left( \frac{0}{0} \right)=\left\{ \text{L'Hôpital} \right\}=\lim_{ x \to 0 }\dfrac{1-\cos x}{9x^{2}+8x^{3}}=\left( \frac{0}{0} \right)=\left\{ \text{L'Hôpital} \right\}=\lim_{ x \to 0 }\dfrac{\sin x}{18x+24x^{2}}=\left( \frac{0}{0} \right)=\left\{ \text{L'Hôpital} \right\}=\lim_{ x \to 0 }\dfrac{\cos x}{18+48x}= \bboxed{\frac{1}{18}}$
\item \lb{Utilizando las reglas de L'Hôpital vamos a calcular los siguientes límites \[ \lim_{x\to\infty}xe^{-x} \]}
$\lim_{x\to+\infty}xe^{-x}=\lim_{x\to+\infty}\dfrac{x}{e^x}=\left(\dfrac{\infty}{\infty}\right)=\left\{\text{L'Hôpital}\right\}=\lim_{x\to+\infty}\dfrac{1}{e^x}=\dfrac{1}{\infty}=\bboxed{0}$
\item \lb{Calcular el polinomio de Taylor de orden 5 de la función $f(x)=e^x$ centrado en el punto $x_0=0$.}

\begin{minipage}[b]{0.58\textwidth}
	\begin{tikzpicture}
		\node[red, draw=red, fill=red!10, line width=1.5, text width=\linewidth] {\underline{Nota:} Cuando calculamos el polinomio de Taylor centrado en $x_0=0$ entonces tiene un nombre especial para este caso, que es desarrollo de McLaurin.};
	\end{tikzpicture}\\
	Como nos piden el desarrollo de McLaurin de orden 5 entonces:
	...
	$\begin{array}{l}
		...
		f''(x)=e^x\rightarrow f''(0)=1\\
		f'''(x)=e^x\rightarrow f'''(0)=1\\
		f^{\mathrm{iv}}(x)=e^x\rightarrow f^{\mathrm{iv}}(0)=1\\
		f^{\mathrm{v}}(x)=e^x\rightarrow f^{\mathrm{iv}}(0)=1\\
	\end{array}\:\bboxed{\tiny T_5(x)=1+x+\tfrac{1}{2}x^2+\tfrac{1}{6}x^3+\tfrac{1}{24}x^4+\tfrac{1}{120}x^5}$
\end{minipage}\quad\begin{minipage}{0.4\textwidth}
\begin{tikzpicture}
	\node[red, draw=red, fill=red!10, line width=1.5, text width=\linewidth] {
	\underline{Nota:} Dada una función $f(x)$, se define su polinomio de Taylor de orden $n$, centrado en un punto $x_0=a$, como:\\
	$T_n(x)=f(a)+\frac{f'(a)}{1!}(x-a)+\frac{f''(a)}{2!}(x-a)^2+\frac{f'''(a)}{3!}(x-a)^3+\cdots+\frac{f^{n}(a)}{n!}(x-a)^n$
	
	Debemos ver que un desarrollo de Taylor es una aproximación de cualquier función mediante un polinomio.  La función y el polinomio son parecidas pero no iguales, por eso existe una expresión que nos permitirá dar una cota del error que se comete en la aproximación. Resto de Lagrange: \[ R_n(x)=\underset{a<c<x}{\left|\dfrac{f^{(n+1)}(c)}{(n+1)!}(x-a)^{n+1}\right|} \]
	};
\end{tikzpicture}
\end{minipage}
\item \lb{Determinar el polinomio de Taylor de grado menor o igual que  de la función $f(x)=\sin x$ en el punto $x_0=\frac{\pi}{2}$.}
...
\item \lb{Hallar el desarrollo de McLaurin de orden 4 de la función $f(x)=\log(1+x)$}
...
\end{array}$
\item \lb{Hallar el desarrollo de McLaurin de orden 5 de la función $f(x)=\sin x$. Utilizando para dar un valor aproximado de $\sin(0.2)$ y acotar el error cometido en dicha aproximación.}
...
\end{minipage}\qquad\begin{minipage}{0.4\textwidth}
\begin{tikzpicture}
	\node[red, draw=red, fill=red!10, line width=1.5, text width=\linewidth] {\underline{Nota:} Siempre que a partir de un desarrollo de Taylor nos piden que luego acotemos el error, lo que haremos será utilizar la expresión del resto de Lagrange, esto implica que necesitaremos hacer una derivada más de las que haremos para Taylor. Si queremos Taylor hasta orden $n$, necesitaremos hacer hasta la derivada $(n+1)$.};
\end{tikzpicture}
\end{minipage}
...
\end{center}
\item \lb{Calcular $\sin(0.3)$ mediante la fórmula de McLaurin. Acotar el error cometido al tomar los siete primeros términos del desarrollo anterior.}
...
Ahora para calcular un valor aproximado de $\sin(0.3)$, lo que haremos será evaluar $T_7(x)$ en $x=0.3$\\


$\begin{array}{l}
	T_7(x=0.3)=0.3-\dfrac{1}{6}(0.3)^3+\dfrac{1}{120}(0.3)^5-\dfrac{1}{5040}(0.3)^7\\
	T_7(x=0.3)=0.2955202\\
	\bboxed{\sin(0.3)\simeq0.2955202}
\end{array}$
\end{minipage}

Para calcular la cota del error cometido en la aproximación, lo que haremos será utilizar el resto de Lagrange: \[ R_7(X)=\left|\dfrac{f^{\mathrm{viii}}(c)}{8!}(x-a)^8\right|=\left|\dfrac{\sin c}{40320}(0.3)^8\right|=\underset{\left|\sin c\right|\le1}{\dfrac{\sin c}{40320}(0.3)^8\le\dfrac{1}{40320}(0.3)^8}=1.627\cdot10^{-9} \]
\fcolorbox{lightblue}{lightblue!10}{La aproximación calculada tiene una cota de error del orden de $E<1.627\cdot10^{-9}$.}

\item \lb{Estudiar la continuidad de la función \[ f(x)=\dfrac{e^{\tan x}-1}{e^{\tan x}+1} \] en $x_0=\dfrac{\pi}{2}$.}

Para empezar $x_0=\dfrac{\pi}{2}\notin\dom(f)$ con lo cual no sería continua en dicho punto. Pero vamos a hacer los límites laterales, para comprobar si, en caso de que sean iguales, podríamos ampliar la función añadiéndole un valor $f\left(\dfrac{\pi}{2}\right)=\lim_{x\to\frac{\pi}{2}}f(x)$ para que sea continua.
...
$\begin{array}{l}
	\lim_{x\to\frac{\pi}{2}^-}f(x)=\lim_{ x \to \frac{\pi}{2}^- }\dfrac{e^{\tan x}-1}{e^{\tan x}+1}=\left( \frac{\infty}{\infty} \right)=\left\{ \text{L'Hôpital} \right\}=\lim_{ x \to \frac{\pi}{2}^- }\dfrac{e^{\tan x}\cdot \frac{1}{\cos^2x}}{e^{\tan x}\cdot \frac{1}{\cos^2x}}=1\\
	\lim_{ x \to \frac{\pi}{2}^+ }f(x)=\lim_{ x \to \frac{\pi}{2}^+ }\dfrac{e^{\tan x}-1}{e^{\tan x}+1}=\left\{ \begin{array}{l}\tan\left( \frac{\pi}{2}^+ \right)=-\infty\\ e^{\tan x}=e^{-\infty}=\frac{1}{e^{+\infty}}=\frac{1}{+\infty}=0\end{array} \right\}=\frac{0-1}{0+1}=-1
\end{array}$

$f(x)$ No en continua en $x_0=\dfrac{\pi}{2}$. En concreto, presenta una discontinuidad inevitable de salto finito o de 1ª especie.

\item \lb{Calcular los extremos absolutos de la función \[ f(x)=(x-2)^2(x+1) \]sobre el conjuto $I=[1,4]$.}

\begin{minipage}{0.4\textwidth}
Como $f(x)$ es continua en $[1,4]$ al ser un polinomio entonces podemos asegurar que por el teorema de Weierstrass $f(x)$ tiene extremos absolutos sobre dicho intervalo.

\begin{center}
$f'(x)=2(x-2)(x+1)+(x-2)^2\cdot1=(x+2)\left[2(x+1)+(x-2)\right]=(x-2)(3x)\linebreak\longrightarrow f'(x)=(x-2)\cdot 3x=0\begin{cases}
x=0&\rc{\mathbf{X}~~(0\notin(1,4))}\\
\bboxed{x=2} & \lb{\checkmark~~(2\in(1,4))}
\end{cases}$
\end{center}
Evaluamos en la función $f(x)$ los puntos críticos: $x=2$ y los dos extremos del intervalo: $x=1$ y $x=4$
\end{minipage}\qquad\begin{minipage}{0.55\textwidth}
\begin{tikzpicture}
	\node[red, draw=red, fill=red!10, line width=1.5, text width=\linewidth] {\underline{Nota:} Siempre que tengamos una función $f(x)$ definida sobre un intervalo cerrado $[a,b]$ y nos pidan que calculemos sus extremos absolutos, haremos lo siguiente:\begin{enumerate}[label=\textbf{\arabic*)}, leftmargin=*]
		\item Justificaremos su existencia mediante el teorema de Weierstrass dejando claro que $f(x)$ es continua sobre el intervalo $[a,b]$.
		\item Hacemos la derivada de $f(x)$ e igualamos a cero, $f'(x)=0$ siendo las soluciones obtenidas los llamados puntos críticos, de entre los cuales solo tendremos en cuenta aquellos que estén en el intervalo $(a,b)$.
		\item Evaluamos en la función $f(x)$ los puntos criticos y los dos extremos del intervalo $(a\text{ y }b)$ y tomaremos como máximo absoluto aquel que tenga mayor imageny como mínimo absoluto el que tenga menor imagen.
	\end{enumerate}};
\end{tikzpicture}
\end{minipage}
...
\begin{tabular}{ll}
$f(1)=1\cdot2=2$ &  \\
$f(2)=0\xrightarrow{\hspace{1.6cm}}$ & \multirow{2}{*}{\fcolorbox{lightblue}{lightblue!10}{\begin{tabular}{l}
$x=2$ está el mínimo absoluto\\
$x=4$ está el máxmo absoluto
\end{tabular}}} \\
$f(4)=4\cdot 5=20\longrightarrow$ & 
\end{tabular}

\item \lb{En el siguiente ejercicio nos piden que calculemos los extremos absolutos de las siguientes funciones en los intervalos indicados:}
...
x=-\dfrac{3\pi}{4}
\end{array}}$
\end{tabular}\qquad\begin{tikzpicture}[baseline=(current bounding box.center), >=latex]
\draw[draw=lightblue, fill=lightblue!10, line width=1.2] (0,0) circle (2);
\draw (-2.5,0) -- (2.5,0);
\draw (0,-2.5) node[below left, blue] {$\cos\left(-\dfrac{3\pi}{4}\right)=\sin\left(-\dfrac{3\pi}{4}\right)=-\dfrac{\sqrt{2}}{2}$} -- (0,2.5);
\draw[draw=blue, fill=blue, line width=1.2, blue] (225:2) circle (2pt) node[below left] {$-\dfrac{3\pi}{4}$} -- (45:2) circle (2pt) node[above right] {$\dfrac{\pi}{4}\qquad\cos\dfrac{\pi}{4}=\sin\dfrac{\pi}{4}=\dfrac{\sqrt{2}}{2}$};
