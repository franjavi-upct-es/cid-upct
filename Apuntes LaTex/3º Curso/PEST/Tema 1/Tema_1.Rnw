...
\textbf{Problema 3.} Sea $(X_t)_{t\in [0,\infty)}$ un proceso estocástico dado por $X_t=Y\cos(U+t)\;\forall t$ donde $Y\sim N(0,1),\,U\sim U(0,2\pi)$ independientes.

Calcular función de medias, varianzas, covarianzas y correlaciones.

\begin{itemize}[label=\textbullet]
    \item \underline{Función de medias:}
        \[
            \mu_X(t)=\mathbb{E}(X_t)=\mathbb{E}(Y\cdot \cos(U+t))=\tozero{\mathbb{E}(Y)}\cdot\mathbb{E}(\cos(U+t))=0\,\forall t
        \] 
    \item \underline{Función de covarianzas:} Sea $t\in [0,\infty)$ y $h\ge 0$ ($h$ representa el salto entre tiempo)
        \[
        \begin{aligned}
        C_X(t,t+h)&=\mathrm{Cov}(X_t,X_{t+h})=\mathrm{Cov}(Y\cos(U+t),Y\cos(U+t+h))\\ 
                  &=\mathbb{E}\left(Y^2\cos(u+t)\cos(U+t+h)\right)-\tozero{\mathbb{E}\left( Y\cos(U+t) \right)} \cdot \mathbb{E}\left( Y\cos(U+t+h) \right)\\
        &=\lbb{\mathbb{E}(Y^2)}{=1}\cdot\mathbb{E} \left( \cos(U+t)\cdot \cos(U+t+h) \right)  \\
        &= \mathbb{E}\left[ \dfrac{1}{2}\left( \cos(2U+2t+h) \right) +\cos(h) \right]  =\dfrac{1}{2}\cos(h)\\
        \end{aligned}
        \] 
        Vamos a calcular: \[
            \begin{aligned}
                \mathbb{E}\left( \cos(2U+2t+h) \right) &=\int_{0}^{2\pi} \cos(2u+2t+h)\cdot \dfrac{1}{2\pi}\du =\left\{ \begin{array}{l}
            v = 2u+2t+h\\
            \dv =2\du 
        \end{array} \right\}=\int_{2t+h}^{4\pi+2t+h}\cos(v)\cdot \dfrac{1}{2\pi}\cdot \dfrac{1}{2}\dv\\ &=\dfrac{1}{4\pi}\cdot \left[ \sin(v) \right] _{2t+h}^{4\pi+2t+h}=0 
        \end{aligned}
        \] 
        Por tanto, $C_X(t,t+h)=\dfrac{1}{2}\cos(h)\quad\forall t\in [0,\infty), h\ge 0$
    \item \underline{Función de varianzas:}
        \[
        \sigma_X^2(t)=\mathrm{Var}(X_t)=C_X(t,t)=\dfrac{1}{2}\cos(0)=\dfrac{1}{2}\quad\forall t
        \] 
    \item \underline{Función de correlaciones:}
        \[
            \rho_X(t,t+h)=\dfrac{C_X(t,t+h)}{\sigma_X(t)\cdot \sigma_X(t+h)}=\dfrac{\frac{1}{2} \cos(h)}{\sqrt{\frac{1}{2}}\sqrt{\frac{1}{2} }  }=\cos(h)\quad \forall t\in [0,\infty), h\ge 0.
        \] 
        \item ¿Es débilmente estacionario?
        
        Sí, porque tiene la función de medias constante y la función de covarianzas (o de correlaciones) depende sólo del salto de tiempo $h$.
\end{itemize}