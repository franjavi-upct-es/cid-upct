...
\subsubsection{Un ejemplo de modelo $ARMA$ con \textbf{\texttt{R}}}

Para ilustrar algunos de los conceptos vistos en este tema, primero vamos a generar una serie temporal a partir de un modelo $ARMA$, proporcionando nosotros mismo tanto los órdenes $p$ y  $q$, como el valor de los coeficientes del modelo. Posteriormente, analizaremos la serie simulada para determinar el modelo generador comprobando que se asemeja al modelo $ARMA$ fijado en la simulación. Evidentemente, cuanto mayor sea el tamaño de la serie simulada, los resultados del ajuste serán más próximos al modelo generador real.

Concretamente, vamos a generar una serie a partir del siguiente modelo $ARMA(p=2,q=1)$:  \[
X_t=0.9X_{t-1}-0.5X_{t-2}+0.7\varepsilon_{t-1}+\varepsilon_t
...
ts.plot(serie_simulada)
@
\caption{Serie simulada}
\end{figure}
...
pacf(serie_simulada)
@
\caption{Correlogramas simple y parcial}
\end{figure}

Observamos que la serie es estacionaria (en media y varianza), además centrada (media cero). En los correlogramas vemos bastantes correlaciones simples y parciales significativas, lo que nos sugiere que el proceso generadr probablemente contenga tanto parte $AR$ (autorregresiva) como parte $MA$ (medias móviles). Nótese que es especialmente significativo el retardo 1 en el correlograma simple y los retaros 1 y 2 en el correlograma parcial, lo que nos sugiere un modelo $ARMA(2,1)$, es decir, un $ARIMA(2,0,1)$.
...
Obtenemos estimaciones de los coeficientes muy parecidos a los coeficientes reales usados en la simulación.
...
<<>>=
modelo_ARIMA202 <- arima(serie_simulada,
                         order = c(2, 0, 2),
                         include.mean = FALSE)
modelo_ARIMA202
@
...
<<>>=
modelo_ARIMA302 <- arima(serie_simulada,
                         order = c(3, 0, 2),
                         include.mean = FALSE)
modelo_ARIMA302
@

Obsérvese que al aumentar el orden de la parte  $AR$ y/o la  $MA$, obtenemos valores de AIC mayores (peores modelos), indicativo de que el modelo óptimo es el  $ARMA(2,1)$.

\subsection{Modelos ARIMA estacionales, SARIMA} 
...
\begin{itemize}[label=\textbullet]
    \item Utilizar gráficos de la serie temporal, de las funciones de autocorrelación (ACF) y autocorrelación parcial (PACF), para determinar los posibles vaores de los órdenes $(p,d,q)$ y $(P,D,Q)$.
    \item Estimar los parámetros del modelo utilizando métodos como la máxima verosimilitud.
    \item Analizar los residuos del modelo asegurarse de que no presentan autocorrelación significativa y que son aproximadamente normales.
\end{itemize}
